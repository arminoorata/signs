...
import { MODE_1_QUESTIONS } from "@/lib/signs/questions";
...
export default function DiagnoseFlow() {
  ...
  const total = MODE_1_QUESTIONS.length;
  if (stepIndex > total) {
    return (
      <ReviewStep
        session={session}
        onSubmit={() => {
          ...
          saveSession(updated);
          router.replace(`/diagnose/results/${sessionId}`);
        }}
        onJumpTo={(i) => router.replace(`/diagnose/flow/${sessionId}/${i + 1}`)}
      />
    );
  }

  const question = MODE_1_QUESTIONS[stepIndex - 1];
  const value =
  ...
  function next() {
    ...
    router.replace(`/diagnose/flow/${sessionId}/${stepIndex + 1}`);
  }
    ...
    <main className="flex-1">
      <section className="max-w-3xl mx-auto px-6 md:px-10 pt-16 md:pt-20 pb-20">
        ...
        <div className="flex items-center justify-between">
          <button
            type="button"
            onClick={() => onJumpTo(MODE_1_QUESTIONS.length - 1)}
            className="text-[13px] text-muted hover:text-foreground transition-colors"
          >
            ← Back
          </button>
